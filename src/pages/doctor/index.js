  ...
  return <Layout title="Sentinel Doctor" description="Private, browser-based LSPDFR log analysis using Project Sentinel diagnostic rules.">
    <main className={styles.page}>
      ...
      <section className={styles.body}><div className="container">
        {intelligencePlan && <IntelligenceCase plan={intelligencePlan} onDismiss={() => {clearDoctorPlan(); setIntelligencePlan(null);}} onStatusChange={(status) => {const updated = updateDoctorCase(intelligencePlan.id, {status}); setIntelligencePlan(updated); setCaseHistory(loadDoctorHistory());}} onUpdate={(changes) => {const updated = updateDoctorCase(intelligencePlan.id, changes); setIntelligencePlan(updated); setCaseHistory(loadDoctorHistory());}} />}
