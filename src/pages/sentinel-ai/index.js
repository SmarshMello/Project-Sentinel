...
function SentinelAIApp(){
  ...
  const [selected,setSelected]=useState(['stop-the-ped','ultimate-backup','compulite','eup-menu']);
  ...
  const chosenPlugins=useMemo(()=>plugins.filter(p=>selected.includes(p.id)).map(p=>({id:p.id,name:p.name,version:p.version,status:p.status,note:p.note})),[selected]);
    ...
    <main className={styles.page}>
      ...
      <section className={styles.workspace}><div className="container"><div className={styles.grid}>
        <aside className={styles.sidebar}>
          ...
          <section className={styles.panel}><div className={styles.panelTitle}>Installed police plugins</div><div className={styles.pluginList}>{plugins.filter(p=>p.sentinelPolice).slice(0,16).map(p=><label className={styles.check} key={p.id}><input type="checkbox" checked={selected.includes(p.id)} onChange={()=>togglePlugin(p.id)}/><span>{p.shortName||p.name}</span></label>)}</div></section>
