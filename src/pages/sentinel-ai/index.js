 ...
 return <Layout title="Sentinel Expert System" description="Free browser-based LSPDFR troubleshooting and log analysis."><main className={styles.page}>
  ...
  <section className={styles.workspace}><div className="container"><div className={styles.grid}>
   <aside className={styles.sidebar}>
    ...
    <section className={styles.panel}><div className={styles.panelTitle}>Diagnostic files</div><FileDropZone compact multiple accept=".log,.txt,.ini,.xml,.json,.cfg" onFiles={addLogs} label="Drop logs or configs here" hint="or click to browse · up to 6 files"/><div className={styles.files}>{logs.map((f,i)=><div key={`${f.name}-${i}`}><span>📄 {f.name}{f.truncated?' (trimmed)':''}</span><button onClick={()=>setLogs(logs.filter((_,n)=>n!==i))}>×</button></div>)}</div><small>Files stay in your browser. Nothing is uploaded to a server.</small></section>
   </aside>
