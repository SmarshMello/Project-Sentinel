...
import {buildDoctorPlan, loadDoctorHistory, saveDoctorPlan} from '@site/src/intelligence/doctorPlanEngine';
import PluginTimeline from './PluginTimeline';
...
export default function ReleaseDetails({profile}) {
  ...
  useEffect(() => {
    setDoctorCases(loadDoctorHistory().filter((item) => item.pluginId === profile.id));
  }, [profile.id]);
