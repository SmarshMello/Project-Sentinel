...
export function answerExpertQuestion(q,report=null,discoveries=[]){const baseProjects=findProjects(q,discoveries),type=intent(q,baseProjects);let projects=attachLive(baseProjects,report);let result={type,projects,mentions:projects,tone:'info',verdict:'Registry answer',summary:'This answer comes from the Sentinel Unified Registry.',watcher:sourceSummary(projects)};
 if(type==='compatibility'){const ids=projects.map(p=>p.id);const hits=conflicts.filter(r=>r.ids.every(id=>ids.includes(id)));const risky=projects.filter(p=>['conflict','legacy','testing','research'].includes(p.status));let reasoning=evaluateProjects(projects,hits);const researchAssessment=compatibilityAssessment(projects,hits);if(researchAssessment){reasoning={...reasoning,confidence:researchAssessment.confidence,recommendation:researchAssessment.recommendation};}result={...result,tone:hits.length?'danger':risky.length?'warning':'success',verdict:hits.length?'Not recommended':projects.length<2?'Name two projects':researchAssessment?.verdict||reasoning.recommendation,summary:hits.length?'Sentinel found a documented conflict or mutually exclusive stack.':projects.length<2?'Add a second project name for a pairwise check.':researchAssessment?.summary||(risky.length?'No direct conflict was found, but one or more projects are not fully verified.':'No hard conflict rule currently matches this combination.'),warnings:[...hits.map(x=>x.reason),...risky.filter(p=>p.status!=='research').map(p=>`${p.name} is classified as ${statusMeta[p.status]?.label||p.status}; test it outside the Golden Build.`)],reasoning,researchAssessment};}
